...
class TXProtoDC2Mock(PipelineStage):
    ...
    def run(self):
        ...
        if self.config['unit_response']:
            if self.rank==0:
                print("Using metacal response R=1 everywhere")
        else:
    ...
    def make_mock_photometry(self, data):
        ...
        photo = make_mock_photometry(n_visit, self.bands, data, unit_response)
        return photo
...
def make_mock_photometry(n_visit, bands, data, unit_response=False):
    """
    ...
    mag_responses = generate_mock_metacal_mag_responses(bands, nobj, unit_response)
...
def generate_mock_metacal_mag_responses(bands, nobj, unit_response):
    import numpy as np
    nband = len(bands)
    mu = np.zeros(nband) # seems approx mean of response across bands, from HSC tract
    if unit_response:
        return np.ones((nband,nobj))
